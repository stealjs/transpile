...
module.exports = function(load, options) {
	...
	var _exports = defaultTo(metadata.exports, []).map(toStealExports);

	return compile(
		`[${moduleId}, function(stealRequire, stealExports) { %= body % }]`
	)({
...
function toStealExports(property) {
	return compile(`stealExports["${property}"] = ${property};`)({}).body[0];
}
